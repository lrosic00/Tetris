...
def draw_text_bottom(text, size, color, surface):
    ...
    surface.blit(label, (top_left_x + play_width/2 - (label.get_width() / 2), top_left_y + play_height/1.5 - label.get_height()/1.5))
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                   while valid_space(current_piece, grid):
                       current_piece.y += 1
                   current_piece.y -= 1
                   print(convert_shape_format(current_piece))  # OVAJ IF ZAKOMENTIRAJ AKO SE CRASHA
...
def main_menu():
    ...
    while run:
        ...
        draw_text_bottom('(controls: arrow keys & spacebar)', 30, (255, 255, 255), win)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
